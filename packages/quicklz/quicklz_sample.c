...
static int quicklz_compress_file(int fd_in, int fd_out)
{
    ...
    rt_memset(state_compress, 0x00, sizeof(qlz_state_compress));
    ...
    for (i = 0; i < file_size; i += COMPRESS_BUFFER_SIZE)
    {
        ...
        rt_memset(buffer, 0x00, COMPRESS_BUFFER_SIZE);
        rt_memset(cmprs_buffer, 0x00, COMPRESS_BUFFER_SIZE + BUFFER_PADDING);
...
static int quicklz_decompress_file(int fd_in, int fd_out)
{
    ...
    rt_memset(state_decompress, 0x00, sizeof(qlz_state_decompress));
    ...
    for (i = 0; i < file_size; i += BLOCK_HEADER_SIZE + block_size)
    {
        ...
        rt_memset(buffer, 0x00, COMPRESS_BUFFER_SIZE + BUFFER_PADDING);
        rt_memset(dcmprs_buffer, 0x00, DCOMPRESS_BUFFER_SIZE);
