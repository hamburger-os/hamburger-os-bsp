                                   ...
                                   pExistingElement->addr.opTrnTopoCnt))
        {
                                              ...
                                              subAddresses.srcIpAddr, msgType))
            {
                ...
                case -1:                     /* List overflow */
                    return TRDP_MEM_ERR;
                case 1:
                    vos_printLog(VOS_LOG_INFO, "Old PD data ignored (SrcIp: %s comId %lu)\n", vos_ipDotted(
    ...
    int         *pIsTSN)
{
    ...
    {   /* Version 1 TRDP (non-TSN) */
        *pIsTSN = FALSE;
        ...
        {
            vos_printLog(VOS_LOG_INFO, "PDframe size error (%lu))\n", packetSize);
            err = TRDP_WIRE_ERR;
        }
        else
        {
            ...
            {
                vos_printLog(VOS_LOG_INFO, "PDframe crc error (%08lx != %08lx))\n", pPacket->frameCheckSum, MAKE_LE(myCRC));
                err = TRDP_CRC_ERR;
            }
    ...
    PD_ELE_T *pSndQueue)
{
    ...
    */
    ...
    if ((vos_cmpTime(&deltaTmax, &temp) == 0) ||
        (noOfPackets == 0))
    {
        vos_printLog(VOS_LOG_INFO, "trdp_pdDistribute: no minimal interval in %ld packets found!\n", noOfPackets);
    ...
    vos_printLog(VOS_LOG_INFO, "trdp_pdDistribute: noOfPackets = %ld\n", noOfPackets);
    ...
    for ((void)(packetIndex = 0), pPacket = pSndQueue; packetIndex < noOfPackets && pPacket != NULL; )
    {
            ...
            (pPacket->interval.tv_usec != 0))
        {
            ...
            {
                vos_printLog(VOS_LOG_INFO, "trdp_pdDistribute: packet [%ld] with interval %lu.%06u could timeout...\n",
            ...
            else
            {
                ...
                vos_printLog(VOS_LOG_INFO, "trdp_pdDistribute: nextTime2Go[%ld] = %lu.%06u\n",
