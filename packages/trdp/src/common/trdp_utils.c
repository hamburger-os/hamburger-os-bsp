        ...
        for (lIndex = 0; lIndex < trdp_getCurrentMaxSocketCnt(iface[0].type); lIndex++)
        {
            if (iface[lIndex].tcpParams.morituri == TRUE)
            {
                ...
                vos_printLog(VOS_LOG_INFO,
                             "Deleting socket from the iface (Sock: %d, lIndex: %ld)\n",
    ...
    TRDP_MSG_T      msgType)
{
    ...
    for (l_index = 0; l_index < pElement->pSeqCntList->curNoOfEntries; ++l_index)
    {
            ...
            (msgType == pElement->pSeqCntList->seq[l_index].msgType))
        {
            ...
            {
                vos_printLog(VOS_LOG_DBG,
                             "Rcv sequence: %lu    last seq: %lu\n",
                             sequenceCounter,
                             pElement->pSeqCntList->seq[l_index].lastSeqCnt);
                vos_printLog(VOS_LOG_DBG, "-> duplicated PD data ignored (SrcIp: %s comId %lu)\n", vos_ipDotted(
                                 srcIP), pElement->addr.comId);
                return 1;
            }
        }
    ...
    vos_printLog(VOS_LOG_DBG, "Rcv sequence: %lu\n", sequenceCounter);
    vos_printLog(VOS_LOG_DBG, "*** new sequence entry (SrcIp: %s comId %lu)\n", vos_ipDotted(
                     srcIP), pElement->addr.comId);
