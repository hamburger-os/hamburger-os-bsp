    ...
    UINT32                  dataSize)
{
    ...
    if (pMsg->comId == TTDB_STATUS_COMID)
    {
            ...
            (dataSize <= sizeof(TRDP_OP_TRAIN_DIR_STATUS_INFO_T)))
        {
            ...
            {
                vos_printLog(VOS_LOG_WARNING, "CRC error of received operational status info (%08lx!= %08lx)!\n",
            ...
            {
                vos_printLog(VOS_LOG_INFO, "ETB topocount changed (old: 0x%08lx, new: 0x%08lx) on %p!\n",
            ...
            if (appHandle->opTrnTopoCnt != appHandle->pTTDB->opTrnState.state.opTrnTopoCnt)
            {
                vos_printLog(VOS_LOG_INFO,
                             "OpTrn topocount changed (old: 0x%08lx, new: 0x%08lx) on %p!\n",
    ...
    UINT32                  dataSize)
{
    ...
    if (pMsg->resultCode == TRDP_NO_ERR)
    {
        ...
        else if (pMsg->comId == TTDB_STAT_CST_REP_COMID)
        {
            ...
            {
                vos_printLog(VOS_LOG_WARNING, "CRC error of received consist info (%08lx != %08lx)!\n",
    ...
    {
        vos_printLog(VOS_LOG_WARNING, "Unsolicited message received (pMsg->comId %lu)!\n", pMsg->comId);
        (void) tlc_setOpTrainTopoCount(appHandle, 0);
        return;
