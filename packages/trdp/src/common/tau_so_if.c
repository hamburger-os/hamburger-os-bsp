    ...
    UINT32                  dataSize)
{
    ...
    vos_printLog(VOS_LOG_INFO, "Message Data received (comId %lu)!\n", pMsg->comId);

    if (pMsg->msgType == TRDP_MSG_ME)
    {
        vos_printLog(VOS_LOG_WARNING, "ME received (comId %lu)!\n", pMsg->comId);
        return;
    }
    if (pMsg->msgType == TRDP_MSG_MR)
    {
        vos_printLog(VOS_LOG_WARNING, "Request timed out (comId %lu)!\n", pMsg->comId);
        return;
    }
    if (pContext == NULL)
    {
        vos_printLog(VOS_LOG_ERROR, "Callback called without context pointer (comId %lu)!\n", pMsg->comId);
        return;
    }
    ...
    {
        vos_printLog(VOS_LOG_WARNING, "Message time out received (comId %lu)!\n", pMsg->comId);
    }
    else
    {
        vos_printLog(VOS_LOG_WARNING, "Error received (comId %lu)!\n", pMsg->comId);
    }
