                                ...
                                BOOL8           checkHeaderOnly)
{
    ...
    {
        vos_printLog(VOS_LOG_ERROR, "MDframe size error (%lu)\n", packetSize);
        err = TRDP_WIRE_ERR;
    }
    ...
    if ( TRDP_NO_ERR == err )
    {
        /* Check Header CRC */
        {
            ...
            {
                vos_printLog(VOS_LOG_ERROR, "MDframe header CRC error. Rcv: %08lx vs %08lx\n",
        ...
        (checkHeaderOnly == FALSE))
    {
        ...
        {
            vos_printLog(VOS_LOG_ERROR, "MDframe invalid length, received %lu, expected %lu\n",
                         packetSize,
                         expectedLength);
            err = TRDP_WIRE_ERR;
        }
    ...
    if (TRDP_NO_ERR == err)
    {
        ...
        {
            vos_printLog(VOS_LOG_WARNING, "Topocount error - received: %lu/%lu, actual: %lu/%lu\n",
    ...
    *pIterMD = NULL;
    ...
    if (state != TRDP_ST_RX_NOTIFY_RECEIVED)
    {
        ...
        for ( iterMD = appHandle->pMDRcvQueue; iterMD != NULL; iterMD = iterMD->pNext )
        {
            ...
            if ( 0 == memcmp(iterMD->pPacket->frameHead.sessionID, pH->sessionID, TRDP_SESS_ID_SIZE))
            {
                                                     ...
                                                     iterMD->addr.opTrnTopoCnt))
                {
                    ...
                    vos_printLog(VOS_LOG_ERROR, "Repeated request topocount error - received: %lu/%lu, expected: %lu/%lu\n",
        ...
        if ( appHandle->mdDefault.maxNumSessions <= numOfReceivers )
        {
            ...
            vos_printLog(VOS_LOG_INFO, "trdp_mdRecv: Max. number of requests reached (%ld)!\n", numOfReceivers);
    ...
    UINT32          sockIndex)
{
    ...
    vos_printLog(VOS_LOG_INFO,
                 "Received %s MD packet (type: '%c%c' UUID: %02x%02x%02x%02x%02x%02x%02x%02x Data len: %lu)\n",
                ...
                /* Compare with the sockets stored in the socket list */
                {
                    ...
                    for (socketIndex = 0; socketIndex < trdp_getCurrentMaxSocketCnt(TRDP_SOCK_MD_UDP); socketIndex++)
                    {
                        if ((appHandle->ifaceMD[socketIndex].sock != VOS_INVALID_SOCKET)
                            ...
                            && (appHandle->ifaceMD[socketIndex].rcvMostly == TRUE))
                        {
                            vos_printLog(VOS_LOG_INFO, "New socket accepted from the same device (Ip = %lu)\n", newIp);

                            if (appHandle->ifaceMD[socketIndex].usage > 0)
                            {
                                vos_printLog(
                                    VOS_LOG_INFO,
                                    "The new socket accepted from the same device (Ip = %lu), won't be removed, because it is still in use\n",
                                    newIp);
                                socketFound = TRUE;
                                break;
                            }
                    ...
                    if (socketFound == FALSE)
                    {
                        ...
                        if (err != TRDP_NO_ERR)
                        {
                            vos_printLog(VOS_LOG_ERROR, "trdp_requestSocket() failed (Err: %d, Port: %ld)\n",
