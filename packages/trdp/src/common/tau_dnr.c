...
static void printDNRcache (TAU_DNR_DATA_T *pDNR)
{
    ...
    {
        vos_printLog(VOS_LOG_DBG, "%03lu:\t%0lu.%0lu.%0lu.%0lu\t%s\t(topo: 0x%08lx/0x%08lx)\n", i,
    ...
    if (fp != NULL)
    {
        ...
        while ((!feof(fp)) && (pDNR->noOfCachedEntries < TAU_MAX_NO_CACHE_ENTRY))
        {
            ...
            if (fgets(line, TAU_MAX_HOSTS_LINE_LENGTH, fp) != NULL)
            {
                ...
                while (l_index < maxIndex && (isdigit((UINT8)line[l_index]) || ispunct((UINT8)line[l_index])))
                {
                    l_index++;
                }

                /* skip the space between IP and URI */
                while (l_index < maxIndex && isspace((UINT8)line[l_index]))
                {
                    l_index++;
                }
                ...
                /* remember start of URI */
                while (l_index < maxIndex && !isspace((UINT8)line[l_index]) && !iscntrl((UINT8)line[l_index]) && line[l_index] != '#')
                {
                    l_index++;
                }
                if (l_index < maxIndex)
        ...
        vos_printLog(VOS_LOG_DBG, "readHostsFile: %ld entries processed\n", pDNR->noOfCachedEntries);
    ...
    for (i = 0u; i < vos_ntohs(dns->ans_count); i++)
    {
        ...
        if (vos_ntohs(answers[i].resource->type) == 1u) /* if its an ipv4 address */
        {
            ...
            *pIP_addr = (TRDP_IP_ADDR_T) ((pReader[0] << 24u) | (pReader[1] << 16u) | (pReader[2] << 8u) | pReader[3]);
            vos_printLog(VOS_LOG_INFO, "%s -> 0x%08lx\n", name, *pIP_addr);
