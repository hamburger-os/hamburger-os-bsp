...
#error \
    "You are trying to compile the swos2 implementation of vos_thread.c - either define swos2 or exclude this file!"
#endif
...
EXT_DECL VOS_ERR_T vos_threadDelay (
    UINT32 delay)
{
    rt_int32_t ms = delay / 1000u;

    rt_thread_mdelay(ms);  //TODO(mingzhao) 等于0的情况
    return VOS_NO_ERR;
}
...
EXT_DECL const CHAR8 *vos_getTimeStamp (void)
{
    ...
    if (curTimeTM != NULL)
    {
        snprintf(pTimeString, sizeof(pTimeString), "%04d%02d%02d-%02d:%02d:%02d.%03ld ",
                      (unsigned int)curTimeTM->tm_year + 1900,
                      (unsigned int)curTimeTM->tm_mon + 1,
                      (unsigned int)curTimeTM->tm_mday,
                      (unsigned int)curTimeTM->tm_hour,
                      (unsigned int)curTimeTM->tm_min,
                      (unsigned int)curTimeTM->tm_sec,
    ...
    VOS_SEMA_STATE_T    initialState)
{
    ...
    else
    {
        ...
        *ppSema = (VOS_SEMA_T) vos_memAlloc(sizeof (struct VOS_SEMA));
        ...
        (*ppSema)->semHandle = rt_sem_create("dsem", 0, RT_IPC_FLAG_PRIO);//TODO(mingzhao) +1
    ...
    UINT32      timeout)
{
    ...
    else if (timeout == 0u)
    {
        ...
        rc = rt_sem_take(sema->semHandle, 0);
    }
    else if (timeout == VOS_SEMA_WAIT_FOREVER)
    {
        /* Take Semaphore, block until Semaphore becomes available */
        rc = rt_sem_take(sema->semHandle, RT_WAITING_FOREVER);
    }
    else
    {
        rc = rt_sem_take(sema->semHandle, timeout / 1000);
    }
    if (0 != rc)
    {
    ...
    VOS_SEMA_T sema)
{
    ...
    {
        /* release semaphore */
        rc = rt_sem_release(sema->semHandle);
        if (0 == rc)
        {
            /* Semaphore released */
        }
