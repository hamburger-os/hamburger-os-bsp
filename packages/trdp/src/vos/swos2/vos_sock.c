...
#ifndef __PLATFORM_SWOS2__
#error \
    "You are trying to compile the swos2 implementation of vos_sock.c - either define swos2 or exclude this file!"
#endif
...
#include <byteswap.h>
#elif defined(__PLATFORM_SWOS2__)
    ...
    UINT32 ipAddress)
{
    ...
    (void)rt_snprintf(dotted, sizeof(dotted), "%u.%u.%u.%u", (ipAddress >> 24), ((ipAddress >> 16) & 0xFF),
...
//netdev_list_if
    ...
    netdev = netdev_get_by_name(pIfName);
    if (netdev == RT_NULL)
    {
        vos_printLogStr("bad network interface device name(%s).\n", pIfName);
        return FALSE;
    }

    rt_memcpy(pMacAddr, netdev->hwaddr, netdev->hwaddr_len);
    ...
    UINT8 pMAC[VOS_MAC_SIZE])
{
    ...
    rt_err_t err = vos_getMacAddress(pMAC, DEFAULT_ETH_NAME);

    return (err == TRUE) ? VOS_NO_ERR : VOS_SOCK_ERR;
}
    ...
    SOCKET sock)
{
    if (close(sock) == -1)
    {
        vos_printLog(VOS_LOG_ERROR,
                     "vos_sockClose(%ld) called with unknown descriptor\n", sock);
        return VOS_PARAM_ERR;
    }
    else
    {
        vos_printLog(VOS_LOG_DBG,
                     "vos_sockClose(%ld) okay\n", sock);
    }
    ...
    BOOL8   peek)
{
    ...
    socklen_t     dlen = sizeof(struct sockaddr);
...
//                               (struct sockaddr *) &dest, (socklen_t *)&dlen);
        ...
        rcvSize = recvfrom(sock, buf, *pSize, 0,
        ...
        if (rcvSize != -1)
        {
            ...
            if (pSrcIPAddr != NULL)
            {
                *pSrcIPAddr = (uint32_t) vos_ntohl(dest.sin_addr.s_addr);
            }

            if (pSrcIPPort != NULL)
            {
                *pSrcIPPort = (UINT16) vos_ntohs(dest.sin_port);
            }
        }
    ...
    UINT16  port)
{
    ...
    if (bind(sock, (struct sockaddr *)&srcAddress, sizeof(struct sockaddr)) == -1)
    {
        char buff[VOS_MAX_ERR_STR_SIZE];
        STRING_ERR(buff);
