...
#define QBOOT_VER_MSG                   "V1.0.6 2022.04.13"
...
static int qbt_dest_part_write(fal_partition_t part, u32 pos, u8 *decmprs_buf, u8 *cmprs_buf, u32 *p_cmprs_len,
        int cmprs_type)
{
    int write_len = 0;
    int cmprs_len = 0;
    ...
    case QBOOT_ALGO_CMPRS_NONE:
        ...
        while(1)
        {
            ...
            rt_memcpy(decmprs_buf, gzip_remain_buf, gzip_remain_len);
            ...
            decomp_len -= gzip_remain_len;
            rt_memcpy(gzip_remain_buf, decmprs_buf + decomp_len, gzip_remain_len);
            if (decomp_len > 0)
            {
            ...
            {
                rt_memset(gzip_remain_buf + gzip_remain_len, 0xFF, GZIP_REMAIN_BUF_SIZE - gzip_remain_len);
    ...
    case QBOOT_ALGO_CMPRS_QUICKLZ:
        while (1)
        {
            ...
            rt_memcpy(cmprs_buf, cmprs_buf + (block_size + QBOOT_QUICKLZ_BLOCK_HDR_SIZE), cmprs_len);
        }
        break;
#endif
        ...
        while(1)
        {
            ...
            rt_memcpy(cmprs_buf, cmprs_buf + (block_size + QBOOT_FASTLZ_BLOCK_HDR_SIZE), cmprs_len);
        }
        break;
#endif
...
static int qbt_app_crc_cal(u32 *p_crc32, u32 max_cal_len, u8 *decmprs_buf, u8 *cmprs_buf, u32 *p_cmprs_len,
        int cmprs_type)
{
    int write_len = 0;
    int cmprs_len = 0;
...
static bool qbt_app_crc_check(const char *fw_part_name, fw_info_t *fw_info)
{
    ...
    while (app_cal_pos < fw_info->raw_size)
    {
        ...
        if (!qbt_fw_pkg_read(src_part, src_read_pos, cmprs_buf + cmprs_len, read_len, crypt_buf, crypt_type))
        {
            ...
            LOG_E("Qboot app crc check fail. read package error, part = %s, addr = %08X, length = %d", fw_part_name,
...
static bool qbt_fw_release(const char *dst_part_name, const char *src_part_name, fw_info_t *fw_info)
{
    ...
    while (dst_write_pos < fw_info->raw_size)
    {
        ...
        {
            qbt_fw_decompress_deinit(cmprs_type);
            LOG_E("Qboot release firmware fail. read package error, part = %s, addr = %08X, length = %d", src_part_name,
                    src_read_pos, read_len);
            return (false);
        }
        src_read_pos += read_len;
        ...
        {
            qbt_fw_decompress_deinit(cmprs_type);
            LOG_E("Qboot release firmware fail. write destination error, part = %s, addr = %08X", dst_part_name,
                    dst_write_pos);
            return (false);
        }
...
static bool qbt_fw_check(const char *fw_part_name, fw_info_t *fw_info, bool output_log)
{
    ...
    {
        if (output_log)
            LOG_E("Qboot firmware check fail. partition %s is not exist.", fw_part_name);
        return (false);
    }
...
RT_WEAK void qbt_jump_to_app(void)
{
    ...
    if ((((u32) app_func & 0xff000000) != 0x08000000) || ((stk_addr & 0x20f00000) != 0x20000000))
    {
        LOG_E("No legitimate application 0x%p 0x%p.", app_func, stk_addr);
        return;
    }

    rt_kprintf("Jump to application 0x%p running ... \n", app_addr);
...
static bool qbt_app_resume_from(const char *src_part_name)
{
    ...
    {
        LOG_E("Qboot resume fail from %s.", src_part_name);
        LOG_E("The firmware of %s partition is not application. fw_info.part_name(%s) != %s", src_part_name,
                fw_info.part_name, QBOOT_APP_PART_NAME);
        return (false);
    }
...
static bool qbt_fw_clone(const char *dst_part_name, const char *src_part_name, u32 fw_pkg_size)
{
    ...
    while (pos < fw_pkg_size)
    {
        ...
        {
            LOG_E("Qboot clone firmware fail. read error, part = %s, addr = %08X, length = %d", src_part_name, pos,
                    read_len);
            return (false);
        }
        if (fal_partition_write(dst_part, pos, cmprs_buf, read_len) < 0)
        {
            LOG_E("Qboot clone firmware fail. write error, part = %s, addr = %08X, length = %d", dst_part_name, pos,
                    read_len);
            return (false);
        }
        pos += read_len;
