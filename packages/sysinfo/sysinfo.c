...
#ifdef RT_USING_FINSH
#include <finsh.h>
MSH_CMD_EXPORT_ALIAS(sysinfofix_show, sysinfofix, Show system info fix.);
#endif

static void sysinfofix_SN_set(int argc, char *argv[])
{
    if (argc < 7)
    {
        rt_kprintf("Usage: SN_set [version] [SN]\n");
        rt_kprintf("       example : SN_set 0 0xFFFFFFFF 0xFFFFFFFF 0xFFFFFFFF 0xFFFFFFFF 0xFFFFFFFF\n");
    }
    else
    {
        uint16_t version = strtoul(argv[1], NULL, 10);
        if (version == 0)//解析为v0
        {
            ...
            SN32[0] = strtoul(argv[2], NULL, 16);
            SN32[1] = strtoul(argv[3], NULL, 16);
            SN32[2] = strtoul(argv[4], NULL, 16);
            SN32[3] = strtoul(argv[5], NULL, 16);
            SN32[4] = strtoul(argv[6], NULL, 16);
...
#ifdef RT_USING_FINSH
#include <finsh.h>
MSH_CMD_EXPORT_ALIAS(sysinfofix_SN_set, SN_set, Set Core Board SN.);
#endif

static void sysinfofix_MAC_set(int argc, char *argv[])
{
    if (argc != 9)
    {
        rt_kprintf("Usage: MAC_set [version] [dev id] [MAC]\n");
        rt_kprintf("       example : MAC_set 0 0 f8 09 a4 32 5d 3e\n");
    }
    else
    {
        uint16_t version = strtoul(argv[1], NULL, 10);
        uint16_t dev_id = strtoul(argv[2], NULL, 10);
        if (version == 0)//解析为v0
        {
            struct SysInfoFixV0Def infofix = {0};
            sysinfofix_get(&infofix);
            infofix.version = version;
            infofix.mac[dev_id][0] = strtoul(argv[3], NULL, 16);
            infofix.mac[dev_id][1] = strtoul(argv[4], NULL, 16);
            infofix.mac[dev_id][2] = strtoul(argv[5], NULL, 16);
            infofix.mac[dev_id][3] = strtoul(argv[6], NULL, 16);
            infofix.mac[dev_id][4] = strtoul(argv[7], NULL, 16);
            infofix.mac[dev_id][5] = strtoul(argv[8], NULL, 16);
            if (sysinfofix_set(&infofix) == 0)
            {
                rt_kprintf("set succeed.\n");
            }
...
#ifdef RT_USING_FINSH
#include <finsh.h>
MSH_CMD_EXPORT_ALIAS(sysinfofix_MAC_set, MAC_set, Set Core Board MAC.);
#endif
