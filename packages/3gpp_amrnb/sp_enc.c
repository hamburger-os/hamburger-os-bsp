...
static void Autocorr(Float32 x[], Float32 r[], const Float32 wind[])
{
    ...
    */
   rt_memset(&y[L_WINDOW], 0, sizeof(Float32) * (M + 1));
...
static void Levinson(Float32 *old_A, Float32 *r, Float32 *A, Float32 *rc)
{
   ...
   rt_memcpy(rc, rct, 4 * sizeof(Float32));
   rt_memcpy(old_A, A, MP1 * sizeof(Float32));
}
...
static void Az_lsp(Float32 a[], Float32 lsp[], Float32 old_lsp[])
{
   ...
   if (nf < M)
   {
      rt_memcpy(lsp, old_lsp, sizeof(Float32) * M);
   }
   return;
}
      ...
      *pred_init_i = 0;
      ...
      for (j = 0; j < PAST_RQ_INIT_SIZE; j++)
      {
         ...
         if (pred_init_err < min_pred_init_err)
         {
            ...
            rt_memcpy(lsf_r1, temp_r1, sizeof(Float32) * M);
            rt_memcpy(lsf_p, temp_p, sizeof(Float32) * M);
            rt_memcpy(past_rq, &past_rq_init[j * M], sizeof(Float32) * M);
            *pred_init_i = j;
         }
      }
   }
                ...
                Float32 *lsp_old_q, Float32 *past_rq, Float32 az[], Float32 azQ[], Float32 lsp_new[], Word16 **anap)
{
   ...
   rt_memcpy(lsp_old, lsp_new, sizeof(Float32) * M);
   rt_memcpy(lsp_old_q, lsp_new_q, sizeof(Float32) * M);
}
                            ...
                            Float32 error[])
{
    ...
    */
   rt_memcpy(ai_zero, Ap1, sizeof(Float32) * MP1);
   ...
   rt_memcpy(exc, res2, sizeof(Float32) * L_SUBFR);
...
static void build_code_2i40_9bits(Word16 subNr, Word32 codvec[], Float32 dn_sign[], Float32 cod[], Float32 h[], Float32 y[], Word16 *anap)
{
   ...
   rt_memset(cod, 0, sizeof(Float32) * L_CODE);
...
static void build_code_8i40_31bits(Word32 codvec[], Float32 dn_sign[], Float32 cod[], Float32 h[], Float32 y[], Word32 sign_indx[], Word32 pos_indx[])
{
   ...
   rt_memset(cod, 0, L_CODE * sizeof(Float32));
...
static void build_code_10i40_35bits(Word32 codvec[], Float32 dn_sign[], Float32 cod[], Float32 h[], Float32 y[], Word16 indx[])
{
   ...
   rt_memset(cod, 0, sizeof(Float32) * L_CODE);
   rt_memset(y, 0, sizeof(Float32) * L_CODE);
                       ...
                       Float32 speech[], Float32 *log_en_hist)
{
   ...
   *hist_ptr += 1;
   ...
   rt_memcpy(&lsp_hist[*hist_ptr * M], lsp_new, sizeof(Float32) * M);
...
static Word32 dtx_enc(Word16 *log_en_index, Float32 log_en_hist[], Float32 lsp_hist[], Word16 *lsp_index, Word32 *init_lsf_vq_index, Word16 compute_sid_flag, Float32 past_rq[], Word32 *past_qua_en, Word16 **anap)
{
   ...
   if ((compute_sid_flag != 0))
   {
       ...
       */
      log_en = 0;
      rt_memset(lsp, 0, sizeof(Float32) * M);
                    ...
                    Word16 ana[], enum Mode *used_mode, Float32 synth[])
{
   ...
   rt_memcpy(st->new_speech, new_speech, sizeof(Float32) * L_FRAME);
   ...
   if (*used_mode == MRDTX)
   {
      ...
      rt_memset(st->old_exc, 0, sizeof(Float32) * (PIT_MAX + L_INTERPOL));
      rt_memset(st->mem_w0, 0, sizeof(Float32) * M);
      rt_memset(st->mem_err, 0, sizeof(Float32) * M);
      rt_memset(st->zero, 0, sizeof(Float32) * L_SUBFR);
      rt_memset(st->hvec, 0, sizeof(Float32) * L_SUBFR);
      rt_memset(st->lspSt->qSt->past_rq, 0, sizeof(Float32) * M);
      rt_memcpy(st->lspSt->lsp_old, lsp_new, sizeof(Float32) * M);
      rt_memcpy(st->lspSt->lsp_old_q, lsp_new, sizeof(Float32) * M);
   ...
   for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
   {
      ...
      {
         rt_memcpy(mem_syn_save, st->mem_syn, sizeof(Float32) * M);
         rt_memcpy(mem_w0_save, st->mem_w0, sizeof(Float32) * M);
         rt_memcpy(mem_err_save, st->mem_err, sizeof(Float32) * M);
         sharp_save = st->sharp;
      }
      ...
      else
      {
         ...
         if (evenSubfr != 0)
         {
            rt_memcpy(h1_sf0, st->h1, sizeof(Float32) * L_SUBFR);
         }
      }

      /* copy the LP residual (res2 is modified in the CL LTP search) */
      rt_memcpy(res2, res, sizeof(Float32) * L_SUBFR);
      ...
      else
      {
         ...
         {
            i_subfr_sf0 = i_subfr;
            rt_memcpy(xn_sf0, xn, sizeof(Float32) * L_SUBFR);
            rt_memcpy(y2_sf0, y2, sizeof(Float32) * L_SUBFR);
            rt_memcpy(code_sf0, code, sizeof(Float32) * L_SUBFR);
            T0_sf0 = T0;
            T0_frac_sf0 = T0_frac;
         ...
         else
         {
             ...
             */
            rt_memcpy(st->mem_err, mem_err_save, sizeof(Float32) * M);
...
static void cod_amr_reset(cod_amrState *s, Word32 dtx)
{
   ...
   /* reset Q_plsfState */
   rt_memset(s->lspSt->qSt->past_rq, 0, sizeof(Float32) * M);
   rt_memcpy(s->lspSt->lsp_old, lsp_init_data, sizeof(lsp_init_data));
   rt_memcpy(s->lspSt->lsp_old_q, lsp_init_data, sizeof(lsp_init_data));
   ...
   rt_memset(s->gainQuantSt->adaptSt->ltpg_mem, 0, sizeof(Float32) * LTPG_MEM_SIZE);
   s->gainQuantSt->sf0_gcode0_exp = 0;
   s->gainQuantSt->sf0_gcode0_fra = 0;
   s->gainQuantSt->sf0_target_en = 0.0F;
   rt_memset(s->gainQuantSt->sf0_coeff, 0, sizeof(Float32) * 5);
   ...
   /* reset tonStabState */
   s->tonStabSt->count = 0;
   rt_memset(s->tonStabSt->gp, 0, sizeof(Float32) * N_FRAME);

   /* reset LevinsonState */
   s->lpcSt->LevinsonSt->old_A[0] = 1.0F;
   rt_memset(&s->lpcSt->LevinsonSt->old_A[1], 0, sizeof(Float32) * M);
   ...
   for (i = 0; i < DTX_HIST_SIZE; i++)
   {
      rt_memcpy(&s->dtxEncSt->lsp_hist[i * M], lsp_init_data, sizeof(Float32) * M);
   }
   rt_memset(s->dtxEncSt->log_en_hist, 0, M * sizeof(Float32));
   ...
   rt_memset(s->old_speech, 0, sizeof(Float32) * L_TOTAL);
   rt_memset(s->old_exc, 0, sizeof(Float32) * (PIT_MAX + L_INTERPOL));
   rt_memset(s->old_wsp, 0, sizeof(Float32) * PIT_MAX);
   rt_memset(s->mem_syn, 0, sizeof(Float32) * M);
   rt_memset(s->mem_w, 0, sizeof(Float32) * M);
   rt_memset(s->mem_w0, 0, sizeof(Float32) * M);
   rt_memset(s->mem_err, 0, sizeof(Float32) * M);
   rt_memset(s->ai_zero, 0, sizeof(Float32) * L_SUBFR);
   rt_memset(s->hvec, 0, sizeof(Float32) * L_SUBFR);
