...
static void Decoder_amr_reset(Decoder_amrState *state, enum Mode mode)
{
   ...
   rt_memset(state->Cb_gain_averState->cbGainHistory, 0, sizeof(Word32) * L_CBGAINHIST);
   ...
   rt_memset(state->old_exc, 0, sizeof(Word32) * (PIT_MAX + L_INTERPOL));

   if (mode != MRDTX)
      rt_memset(state->mem_syn, 0, sizeof(Word32) * M);
   ...
   if (mode != MRDTX)
      rt_memset(state->excEnergyHist, 0, sizeof(Word32) * 9);
   rt_memset(state->ltpGainHistory, 0, sizeof(Word32) * 9);
   ...
   rt_memset(state->lsfState->past_r_q, 0, sizeof(Word32) * M);
   ...
   /* Static vectors to zero */
   rt_memset(state->background_state->frameEnergyHist, 0, sizeof(Word32) * L_ENERGYHIST);

   /* Initialize hangover handling */
   state->background_state->bgHangover = 0;

   /* phDispReset */
   rt_memset(state->ph_disp_st->gainMem, 0, sizeof(Word32) * PHDGAINMEMSIZE);
   ...
   if (mode != MRDTX)
   {
      ...
      {
         rt_memcpy(&state->dtxDecoderState->lsf_hist[M * i], &state->dtxDecoderState->lsf_hist[0], sizeof(Word32) * M);
      }
      rt_memset(state->dtxDecoderState->lsf_hist_mean, 0, sizeof(Word32) * M * DTX_HIST_SIZE);
...
static void D_plsf_3(D_plsfState *st, enum Mode mode, Word16 bfi, Word16 *indice, Word32 *lsp1_q)
{
   ...
   else
   {
      ...
      if (mode != MRDTX)
      {
         ...
         rt_memcpy(st->past_r_q, lsf1_r, sizeof(Word32) * M);
      }
      else
      {
         for (i = 0; i < M; i++)
   ...
   rt_memcpy(st->past_lsf_q, lsf1_q, sizeof(Word32) * M);
...
static void A_Refl(Word32 a[], Word32 refl[])
{
   ...
   /* initialize states */
   rt_memcpy(aState, a, sizeof(Word32) * M);
   ...
   for (i = M - 1; i >= 0; i--)
   {
      ...
      rt_memcpy(aState, bState, sizeof(Word32) * i);
   }
   return;
ExitRefl:
   rt_memset(refl, 0, sizeof(Word32) * M);
}
...
static void Build_CN_code(Word32 *seed, Word32 cod[])
{
   ...
   rt_memset(cod, 0, sizeof(Word32) * L_SUBFR);
...
static Word32 Syn_filt(Word32 a[], Word32 x[], Word32 y[], Word32 lg, Word32 mem[], Word32 update)
{
   ...
   /* Copy mem[] to yy[] */
   rt_memcpy(tmp, mem, sizeof(Word32) * M);
   yy = tmp + M;
   yy_limit = yy + lg;
   a0 = a[0];
   ...
   rt_memcpy(y, &tmp[M], sizeof(Word32) * lg);

   /* Update of memory if update==1 */
   if (update)
   {
      rt_memcpy(mem, &y[lg - M], sizeof(Word32) * M);
   }
   return overflow;
}
                    ...
                    Word32 A_t[])
{
   ...
   if ((st->dtxHangoverAdded != 0) & (st->sid_frame != 0))
   {
      ...
      rt_memcpy(&st->lsf_hist[ptr], &st->lsf_hist[st->lsf_hist_ptr], sizeof(Word32) * M);
       ...
       */
      st->log_en = 0;
      rt_memset(lsf, 0, sizeof(Word32) * M);
      ...
      rt_memcpy(st->lsf_hist_mean, st->lsf_hist, sizeof(Word32) * 80);
   ...
   if (st->sid_frame != 0)
   {
       ...
       */
      rt_memcpy(st->lsp_old, st->lsp, sizeof(Word32) * M);
      ...
      if (st->valid_data != 0) /* new data available (no CRC) */
      {
         ...
         rt_memcpy(lsfState->past_r_q, &past_rq_init[parm[0] * M], sizeof(Word32) * M);
         D_plsf_3(lsfState, MRDTX, 0, &parm[1], st->lsp);

         /* reset for next speech frame */
         rt_memset(lsfState->past_r_q, 0, sizeof(Word32) * M);
         log_en_index = parm[4];
   ...
   rt_memcpy(lsf_int_variab, lsf_int, sizeof(Word32) * M);
   ...
   rt_memcpy(lsfState->past_lsf_q, lsf_int, sizeof(Word32) * M);
   ...
   rt_memcpy(&A_t[0], acoeff, sizeof(Word32) * MP1);
   rt_memcpy(&A_t[MP1], acoeff, sizeof(Word32) * MP1);
   rt_memcpy(&A_t[MP1 << 1], acoeff, sizeof(Word32) * MP1);
   rt_memcpy(&A_t[MP1 + MP1 + MP1], acoeff, sizeof(Word32) * MP1);
...
static void D_plsf_5(D_plsfState *st, Word16 bfi, Word16 *indice, Word32 *lsp1_q, Word32 *lsp2_q)
{
   ...
   if (bfi != 0)
   {
      ...
      rt_memcpy(lsf2_q, lsf1_q, sizeof(Word32) * M);
   ...
   rt_memcpy(st->past_lsf_q, lsf2_q, sizeof(Word32) * M);
...
static void decode_8i40_31bits(Word16 index[], Word32 cod[])
{
   ...
   rt_memset(cod, 0, sizeof(Word32) * L_CODE);
...
static void Int_lsf(Word32 lsf_old[], Word32 lsf_new[], int i_subfr, Word32 lsf_out[])
{
   ...
   case 120:
      rt_memcpy(lsf_out, lsf_new, sizeof(Word32) * M);
      break;
   }
}
                    ...
                    Word32 pitch_fac, Word32 tmp_shift)
{
   ...
   if ((mode != MR122) & (mode != MR102) & (mode != MR74) & (impNr < 2))
   {
      ...
      rt_memcpy(inno_sav, inno, sizeof(Word32) * L_SUBFR);
      rt_memset(inno, 0, sizeof(Word32) * L_SUBFR);
...
static void dtx_dec_activity_update(dtx_decState *st, Word32 lsf[], Word32 frame[])
{
   ...
   rt_memcpy(&st->lsf_hist[st->lsf_hist_ptr], lsf, sizeof(Word32) * M);
                        ...
                        enum RXFrameType frame_type, Word32 synth[], Word32 A_t[])
{
   ...
   rt_memcpy(prev_lsf, st->lsfState->past_lsf_q, sizeof(Word32) * M);
   ...
   rt_memcpy(st->lsp_old, lsp_new, sizeof(Word32) * M);
   ...
   for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
   {
       ...
       */
      rt_memcpy(exc_enhanced, st->exc, sizeof(Word32) * L_SUBFR);
      ...
      {
         rt_memcpy(st->mem_syn, &synth[i_subfr + 30], sizeof(Word32) * M);
      }
       ...
       */
      rt_memcpy(&st->old_exc[0], &st->old_exc[L_SUBFR], sizeof(Word32) * (PIT_MAX + L_INTERPOL));
                        ...
                        Word32 *Az_4)
{
    ...
    * Post filtering
    */
   rt_memcpy(syn_work, syn, sizeof(Word32) * L_FRAME);
   Az = Az_4;
   ...
   for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
   {
      ...
      rt_memcpy(h, Ap3, sizeof(Word32) * MP1);
      rt_memset(&h[M + 1], 0, sizeof(Word32) * (22 - M - 1));
      ...
      *p1 = *p1 - ((temp2 * st->preemph_state_mem_pre) >> 15);
      ...
      {
         rt_memcpy(st->mem_syn_pst, &syn[i_subfr + 30], sizeof(Word32) * M);
      }
   ...
   rt_memcpy(&syn_work[-M], &syn_work[L_FRAME - M], sizeof(Word32) * M);
   return;
}
...
static int Decoder_amr_init(Decoder_amrState **state)
{
   ...
   *state = NULL;
   ...
   rt_memset(s->Cb_gain_averState->cbGainHistory, 0, sizeof(Word32) * L_CBGAINHIST);
...
static int Post_Filter_reset(Post_FilterState *state)
{
   ...
   rt_memset(state->mem_syn_pst, 0, sizeof(Word32) * M);
   rt_memset(state->res2, 0, sizeof(Word32) * L_SUBFR);
   rt_memset(state->synth_buf, 0, sizeof(Word32) * (L_FRAME + M));
   return 0;
}
