...
static void encoder(int argc, char *argv[])
{
    ...
    // set file_mode_path
    rt_strcpy(file_mode_path, argv[2]);
    // set file_speech_path
    rt_strcpy(file_speech_path, argv[3]);
    // set file_encoded_path
    rt_strcpy(file_encoded_path, argv[4]);

    // create thread
    rt_thread_t thread = rt_thread_create("encoder", encoder_thread, RT_NULL, 128 * 1024, 21, 10);
    if (thread != RT_NULL)
