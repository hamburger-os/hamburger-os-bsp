 ...
 *  STATIC PROTOTYPES
 **********************/
static void lv_file_explorer_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj) LV_SECTION;

static void browser_file_event_handler(lv_event_t * e) LV_SECTION;
#if LV_FILE_EXPLORER_QUICK_ACCESS
    static void quick_access_event_handler(lv_event_t * e) LV_SECTION;
    static void quick_access_area_event_handler(lv_event_t * e) LV_SECTION;
#endif

static void init_style(lv_obj_t * obj) LV_SECTION;
static void show_dir(lv_obj_t * obj, const char * path) LV_SECTION;
static void strip_ext(char * dir) LV_SECTION;
static void file_explorer_sort(lv_obj_t * obj) LV_SECTION;
static void sort_by_file_kind(lv_obj_t * tb, int16_t lo, int16_t hi) LV_SECTION;
static void exch_table_item(lv_obj_t * tb, int16_t i, int16_t j) LV_SECTION;
static bool is_end_with(const char * str1, const char * str2) LV_SECTION;
...
void lv_file_explorer_set_quick_access_path(lv_obj_t * obj, lv_file_explorer_dir_t dir, const char * path)
{
    ...
    size_t nsize = strlen(path);
    char *news = (char *)malloc(nsize + 1);
    if (news)
    {
        rt_memcpy(news, path, nsize);
        news[nsize] = '\0';
    }
...
static void lv_file_explorer_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    memset(explorer->current_path, 0x00, sizeof(explorer->current_path));
...
static void show_dir(lv_obj_t * obj, const char * path)
{
    ...
    memset(explorer->current_path, 0x0, sizeof(explorer->current_path));
