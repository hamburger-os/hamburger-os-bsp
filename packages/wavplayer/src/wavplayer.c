...
static rt_err_t wavplayer_open(struct wavplayer *player)
{
    ...
    LOG_I("Information: samplerate %d, channels %d, samplebits %d"
            , wav.fmt_sample_rate, wav.fmt_channels, wav.fmt_bit_per_sample);
...
static void wavplayer_entry(void *parameter)
{
    ...
    /* set volume */
    wavplayer_volume_set(player.volume);
    ...
    while (1)
    {
        ...
        {
            player.state = PLAYER_STATE_STOPED;
            LOG_E("open wav player failed");
            continue;
        }

        LOG_I("play start, device %s, uri=%s", PKG_WP_PLAY_DEVICE, player.uri);
        while (1)
        {
        ...
        LOG_I("play end, use %d.%d s", player.start/1000, player.start%1000);
    }
