...
__STATIC_INLINE void PORT_SWD_SETUP (void) {
    rt_base_t swclk = rt_pin_get(CMSIS_DAP_SWCLK_TCK_PIN);
    rt_pin_mode(swclk, PIN_MODE_OUTPUT);
    rt_pin_write(swclk, PIN_HIGH);

    rt_base_t swdio_out = rt_pin_get(CMSIS_DAP_SWDIO_OUT_PIN);
    rt_pin_mode(swdio_out, PIN_MODE_OUTPUT);
    rt_pin_write(swdio_out, PIN_HIGH);

    rt_base_t swdio_in = rt_pin_get(CMSIS_DAP_SWDIO_IN_PIN);
    rt_pin_mode(swdio_in, PIN_MODE_INPUT);
}
...
__STATIC_INLINE void PORT_OFF (void) {
    rt_base_t swclk = rt_pin_get(CMSIS_DAP_SWCLK_TCK_PIN);
    rt_pin_mode(swclk, PIN_MODE_INPUT);

    rt_base_t swdio_out = rt_pin_get(CMSIS_DAP_SWDIO_OUT_PIN);
    rt_pin_mode(swdio_out, PIN_MODE_INPUT);
...
__STATIC_FORCEINLINE void     PIN_SWDIO_TMS_CLR (void) {
    rt_base_t swdio_out = rt_pin_get(CMSIS_DAP_SWDIO_OUT_PIN);
    rt_pin_write(swdio_out, PIN_LOW);
}
...
__STATIC_INLINE void LED_CONNECTED_OUT (uint32_t bit) {
    rt_base_t com = rt_pin_get(CMSIS_DAP_COM_LED_PIN);
    if (bit & 1)
        rt_pin_write(com, PIN_LOW);
    else
        rt_pin_write(com, PIN_HIGH);
}
...
__STATIC_INLINE void LED_RUNNING_OUT (uint32_t bit) {
    rt_base_t dap = rt_pin_get(CMSIS_DAP_DAP_LED_PIN);
    if (bit & 1)
        rt_pin_write(dap, PIN_LOW);
    else
        rt_pin_write(dap, PIN_HIGH);
}
...
__STATIC_INLINE void DAP_SETUP (void) {
    ...
    rt_base_t com = rt_pin_get(CMSIS_DAP_COM_LED_PIN);
    rt_pin_mode(com, PIN_MODE_OUTPUT);
    rt_pin_write(com, PIN_LOW);

    rt_base_t dap = rt_pin_get(CMSIS_DAP_DAP_LED_PIN);
    rt_pin_mode(dap, PIN_MODE_OUTPUT);
    rt_pin_write(dap, PIN_LOW);
}
