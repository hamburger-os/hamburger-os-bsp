...
static uint8_t sc16is7xx_port_read(struct rt_spi_device *port, uint8_t reg)
{
    rt_err_t ret = RT_EOK;

    uint8_t val = 0;
    ret = rt_spi_send_then_recv(port, &reg, 1, &val, 1);
    if (ret != RT_EOK)
    {
        LOG_E("port read error %d!", ret);
    }

    return val;
}

static void sc16is7xx_port_write(struct rt_spi_device *port, uint8_t reg, uint8_t val)
{
    uint8_t cmd[] = {reg, val};
    rt_size_t size = rt_spi_send(port, cmd, sizeof(cmd));
    if (size != sizeof(cmd))
    {
        LOG_E("port write error");
    }
}
