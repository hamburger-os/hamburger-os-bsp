...
typedef __packed union
{
  ...
  {
    rt_uint8_t FamilyCode;
    rt_uint8_t SerialCode[6];
    rt_uint8_t CRC_byte;
  } Code;
  rt_uint8_t u8p_Data[8];
} U_DS31820ID;
...
typedef union
{
  ...
  struct
  {
    rt_uint8_t TempL;    /*温度低字节*/
    rt_uint8_t TempH;    /*温度的高字节*/
    rt_uint8_t User1;    /*用户存储区1*/
    rt_uint8_t User2;    /*用户存储区2*/
    rt_uint8_t Confg;    /*配置存储区*/
    rt_uint8_t ResFF;    /*预留*/
    rt_uint8_t Res;      /*预留*/
    rt_uint8_t Res10;    /*预留*/
    rt_uint8_t CRC_byte; /*以上字节的CRC8 */
  } s_Memory;
  rt_uint8_t a_data[9]; /*数据*/
} U_MAX31826Memory;
...
static rt_uint8_t crc8_create(rt_uint8_t *data, rt_uint16_t number_of_bytes_in_data)
{
  rt_uint8_t crc;
  uint16_t loop_count;
  rt_uint8_t bit_counter;
  rt_uint8_t b;
  rt_uint8_t feedback_bit;
...
int Max3182x_ReadTemp(rt_uint16_t *u16p_temp_value)
{
  ...
  else
  {
    ...
    u16_Temp = (uint16_t)(u8_tempH & ((rt_uint8_t)0x0F));
    u16_Temp <<= 8;
...
static rt_int32_t DEV_MAX31826_ReadID(rt_uint8_t *bufferid)
{
  ...
  if (DEV_MAX31826_Reset1Wire())
  {
    ...
    {
      LOG_D("id : %x %x %x %x %x %x", p_id[0], p_id[1], p_id[2], p_id[3], p_id[4], p_id[5], p_id[6]);
      ret = (rt_int32_t)0;
    }
    else
    {
      LOG_E("id : %x %x %x %x %x %x", p_id[0], p_id[1], p_id[2], p_id[3], p_id[4], p_id[5], p_id[6]);
      ret = (rt_int32_t)-1;
    }
  }
  return ret;
...
/* *******************************************
功    能:向温度传感器内部存储区域写入8个字节数据
参    数:TempSensorSelect:指定温度传感器;
            offset:内部存储区的起始偏移;
            buf:写缓冲区起始地址.
返       回:成功:1      失败:0
******************************************* */
int Max3182x_Write8Bytes(rt_uint8_t offset, rt_uint8_t *buf)
{
  int e_return = ER;
  rt_uint8_t i;
  rt_uint8_t a_tmp[11];
  ...
  /* 将要写入的字节放到数组中  */
  a_tmp[0] = MAX31826_WRITE_SCRATCHPAD2;
  a_tmp[1] = offset;
  rt_memcpy((void *)&a_tmp[2], (void *)buf, (uint16_t)8);

  /* 复位传感器,按照时序进行操作 */
  DEV_MAX31826_Reset1Wire();
  DEV_MAX31826_Write1Wire(MAX31826_CMD_SKIP_ROM);

  for (i = (rt_uint8_t)0; i < (rt_uint8_t)10; i++)
  {
    DEV_MAX31826_Write1Wire(a_tmp[i]);
  }
  /* 第十一个字节是传感器输出刚写入的10个字节的CRC */
  a_tmp[10] = DEV_MAX31826_Read1Wire();

  /* 校验序列 */
  if (crc8_create(a_tmp, (uint16_t)11, 0) == (rt_uint8_t)0)
  {
    /* 发送写入命令 */
    DEV_MAX31826_Reset1Wire();
    DEV_MAX31826_Write1Wire(MAX31826_CMD_SKIP_ROM);
...
/* *******************************************
  功    能:向温度传感器内部存储区域写入指定长度数据
  参    数:TempSensorSelect:指定温度传感器;
            offset:内部存储区的起始偏移;
            len:缓冲区长度;
            buf:写缓冲区起始地址.
  返 回 值:SUCCESS:成功;ERROR:错误
******************************************* */
int fal_max31826_write(long offset, const rt_uint8_t *buf, size_t len)
{
  rt_uint8_t a_temp_buf[8];
  int e_return = ER;
  rt_uint8_t remain_len, write_len, offset_tmp, error_times;
  error_times = (rt_uint8_t)0;

  /* 起始地址是8的整数倍   */
  if ((offset % (rt_uint8_t)8) != (rt_uint8_t)0)
  {
    return ER;
  }
  ...
  while (remain_len > (rt_uint8_t)0)
  {
    ...
    if (remain_len > (rt_uint8_t)8)
    {
      write_len = (rt_uint8_t)8;
      memcpy((void *)a_temp_buf, (void *)&buf[offset_tmp], (uint16_t)8);
    }
    else
    {
      /* 不足8个字节的补零 */
    ...
    if (Max3182x_Write8Bytes((rt_uint8_t)(offset + offset_tmp), a_temp_buf) == OK)
    {
