...
#define CONFIG_USBRX_RINGBUF_SIZE  (1024)
...
static void chry_dap_init(void)
{
...
static void dap_thread_entry(void *parameter)
{
    chry_dap_init();
    while (1)
    {
...
{
    /* 创建 dap 线程 */
    rt_thread_t thread = rt_thread_create("dap", dap_thread_entry, NULL, 1024, 10, 10);
    /* 创建成功则启动线程 */
    if (thread != RT_NULL)
    {
...
/* 导出到自动初始化 */
INIT_SERVICE_EXPORT(cherryusb_device_init);
