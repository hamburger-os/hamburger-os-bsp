...
void selftest_uart_test(SelftestlUserData *puserdata)
{
    ...
    for (int i = 0; i<3; i++)
    {
        rt_device_read(puserdata->uart_dev[i][1], 0, data_rd[i], 8);
        rt_device_write(puserdata->uart_dev[i][0], 0, data_wr, 8);
        rt_thread_delay(100);

        rt_memset(data_rd[i], 0, 8);
        data_rd_len = rt_device_read(puserdata->uart_dev[i][1], 0, data_rd[i], 8);
        if (data_rd_len != 8)
        {
            LOG_E("%s read failed %d", error_log[i], data_rd_len);
        }
        if (rt_memcmp(data_wr, data_rd[i], 8) == 0)
        {
