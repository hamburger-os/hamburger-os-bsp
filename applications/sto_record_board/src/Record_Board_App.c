    ...
    else
    {
			...
			if (0 == total_packet)
      {
        ...
        else if (LocomotiveInfo_Type == curve_type)
        {
					...
					printf("LocomotiveInfo len is Zero\n");
					printf("s_packet_gradeInfo.locomotiveinfo_received_length_u16 = 0,total_packet = 0\n");
				}
			}
		}
...
//		printf("grade: packet_id = %d total_packet = %d total_length = %d\n",pack_id,total_packet,s_packet_gradeInfo.total_length_u16);
  ...
  {
		/* 判断组号和帧号是否匹配 */
		if ((s_packet_gradeInfo.group_id_u8 == ps_can_frame->data_u8[0]) && (s_packet_gradeInfo.frame_id_u8 == ps_can_frame->no_u8))
    {
			for (i = 1;i < 8;i++)
      {
				s_packet_gradeInfo.receiving_data_u8[s_packet_gradeInfo.receiving_length_u16++] = ps_can_frame->data_u8[i];
			}
			...
			if ((s_packet_gradeInfo.group_id_u8 == end_group) && (s_packet_gradeInfo.frame_id_u8 == end_frame))
      {
				...
				crc_check2 = TO_UINT16(s_packet_gradeInfo.receiving_data_u8[1],s_packet_gradeInfo.receiving_data_u8[0]);

				if (crc_check1 != crc_check2)
        {
					/* 发送错误返回帧 */
					printf("grade: CRC error\n");
#if 0
...
//				printf("can_receive_gradepacket length:%d\n",length);
        ...
        else if (LocomotiveInfo_Type == curve_type)
        {
					...
					for (j = 0;j < length;j++)
          {
						s_packet_gradeInfo.received_data_u8[s_packet_gradeInfo.locomotiveinfo_received_length_u16 + j] = s_packet_gradeInfo.receiving_data_u8[j + 2];
					}
...
//						s_packet_gradeInfo.locomotiveinfo_received_length_u16 = s_packet_gradeInfo.received_length_u16;
		...
		else
    {
			...
			if ((end_group == ps_can_frame->data_u8[0]) && ((uint8_t)s_pack_describe.end_frm == ps_can_frame->no_u8))
      {
        ...
        {
					printf("grade: file error count_out\n");
					file_error_count = 0;
					s_packet_gradeInfo.e_state = E_IDLE;
				}
			}
