...
sint32_t fm_free_fram_space(S_FILE_MANAGER *fm)
{
    ...
    {
        if (stat_l.st_size < (TMP_FILE_MAN_SIZE * 1024))
        {
//            LOG_I("<< %s", fm->latest_tmp_file_info.file_name);
            /* 当前文件未超过文件设置的大小 */
            return 0;
        }
        else /* 当前文件超过文件设置的大小 */
...
//            LOG_I(">> %s", fm->latest_tmp_file_info.file_name);
            ...
            else
            {
                ...
                LOG_I("free fram after: %d K", disk_free_space);
                return ret;
            }
        }
    }
