...
#define DBG_TAG "trdp_pd_pattern_telegram_test"
...
#define TRDP_PUSH_PATTERN_TELEGRAM_TEST_PORT_NUM (6)
...
typedef struct {
    ...
    uint32_t pub_commid;
    uint32_t sub_commid;
    Port ports[TRDP_PUSH_PATTERN_TELEGRAM_TEST_PORT_NUM];   /* array of ports          */
...
static void trdp_pd_push_pattern_telegram_test_thread(void *paramemter)
{
...
/* --- trdp_pd_test ------------------------------------------------------------------*/
void trdp_pd_push_pattern_telegram_test(int argc, char * argv[])
{
    TRDP_ERR_T err;
    rt_thread_t tid;
    ...
    if (argc < 3)
    {
        ...
        rt_kprintf("<mcast>         .. multicast group address (ie. 239.255.3.0)\n");
        rt_kprintf("<pub comm id>   .. pub comm id (ie. 300)\n");
        return;
    }
    ...
    tid = rt_thread_create("trdp_pd_push_pattern_telegram_test",
                          trdp_pd_push_pattern_telegram_test_thread, RT_NULL,
                        (1024 * 6), 20, 5);
    if (tid != RT_NULL)
    {
...
#include <finsh.h>
MSH_CMD_EXPORT_ALIAS(trdp_pd_push_pattern_telegram_test, trdp_pd_push_pattern_telegram_test, trdp pd push pattern telegram test);
#endif
