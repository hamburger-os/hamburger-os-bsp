...
#define DBG_TAG "trdp_pd_data_length_test"
...
#define TRDP_PUSH_DATA_LENGTH_TEST_PORT_NUM (6)
...
typedef struct {
    ...
    Port ports[TRDP_PUSH_DATA_LENGTH_TEST_PORT_NUM];   /* array of ports          */
...
static void trdp_pd_data_length_test_thread(void *paramemter)
{
...
/* --- trdp_pd_data_length_test ------------------------------------------------------------------*/
void trdp_pd_data_length_test(int argc, char * argv[])
{
    TRDP_ERR_T err;
    rt_thread_t tid;
    ...
    if (argc < 4)
    {
        ...
        rt_kprintf("<mcast>         .. multicast group address (ie. 239.255.3.24/239.255.3.25/239.255.3.26)\n");
        rt_kprintf("<pub comm id>   .. pub comm id (ie. 324/325/326)\n");
        rt_kprintf("<data length>   .. data lendth (ie. 1432/0/512)\n");
        return;
    }
    ...
    tid = rt_thread_create("trdp_pd_data_length_test_thread",
                            trdp_pd_data_length_test_thread, RT_NULL,
                        (1024 * 6), 20, 5);
    if (tid != RT_NULL)
    {
...
#include <finsh.h>
MSH_CMD_EXPORT_ALIAS(trdp_pd_data_length_test, trdp_pd_data_length_test, trdp data length test);
#endif
