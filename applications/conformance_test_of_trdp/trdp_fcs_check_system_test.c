...
/*
 * 测试时需要将trdp_pdUpdate函数中计算CRC的初始值修改，制造FCS错误的示例
 * */
...
#define DBG_TAG "trdp_pd_fcs_check_system_test"
...
#define TRDP_PUSH_FCS_CHECK_SYSTEM_TEST_PORT_NUM (6)

#define TRDP_PUSH_FCS_CHECK_SYSTEM_TEST_PUB_MCAST ("239.255.3.2")
#define TRDP_PUSH_FCS_CHECK_SYSTEM_TEST_SUB_MCAST ("239.255.3.3")

#define TRDP_PUSH_FCS_CHECK_SYSTEM_TEST_PUB_COM_ID (302)
#define TRDP_PUSH_FCS_CHECK_SYSTEM_TEST_SUB_COM_ID (303)
...
typedef struct {
    ...
    Port ports[TRDP_PUSH_FCS_CHECK_SYSTEM_TEST_PORT_NUM];   /* array of ports          */
...
static void trdp_pd_fcs_check_system_thread(void *paramemter)
{
...
/* --- trdp_pd_push_fcs_check_system_test ------------------------------------------------------------------*/
void trdp_pd_push_fcs_check_system_test(int argc, char * argv[])
{
    TRDP_ERR_T err;
    rt_thread_t tid;
    ...
    trdp_pd_test_dev.ip_cfg.pub_mcast = vos_dottedIP(TRDP_PUSH_FCS_CHECK_SYSTEM_TEST_PUB_MCAST);   /* 发布者 组播地址 */
    trdp_pd_test_dev.ip_cfg.sub_mcast = vos_dottedIP(TRDP_PUSH_FCS_CHECK_SYSTEM_TEST_SUB_MCAST);   /* 订阅者 组播地址 */
    trdp_pd_test_dev.pub_com_id = TRDP_PUSH_FCS_CHECK_SYSTEM_TEST_PUB_COM_ID;         /* 发布者 commid */
    trdp_pd_test_dev.sub_com_id = TRDP_PUSH_FCS_CHECK_SYSTEM_TEST_SUB_COM_ID;         /* 订阅者 commid */
    ...
    tid = rt_thread_create("trdp_pd_fcs_check_system_thread",
                            trdp_pd_fcs_check_system_thread, RT_NULL,
                        (1024 * 6), 20, 5);
    if (tid != RT_NULL)
    {
...
#include <finsh.h>
MSH_CMD_EXPORT_ALIAS(trdp_pd_push_fcs_check_system_test, trdp_pd_push_fcs_check_system_test, trdp pd push fcs check system test);
#endif
