...
#define DBG_TAG "trdp_pd_point_to_point_test"
...
#define TRDP_PUSH_POINT_TO_POINT_TEST_PORT_NUM (6)

#define TRDP_PD_POINT_TO_PONIT_TEST_LOCAL_IP    ("10.0.1.1")
#define TRDP_PD_POINT_TO_PONIT_TEST_REMOTE_IP   ("10.0.1.2")
...
typedef struct {
    ...
    uint32_t pub_commid;
    uint32_t sub_commid;
    Port ports[TRDP_PUSH_POINT_TO_POINT_TEST_PORT_NUM];   /* array of ports          */
...
static void trdp_pd_point_to_point_test_thread(void *paramemter)
{
...
/* --- trdp_pd_test ------------------------------------------------------------------*/
void trdp_pd_point_to_point_test(int argc, char * argv[])
{
    TRDP_ERR_T err;
    rt_thread_t tid;
    ...
    trdp_pd_test_dev.ip_cfg.srcip = vos_dottedIP(TRDP_PD_POINT_TO_PONIT_TEST_LOCAL_IP);       /* 本地IP */
    trdp_pd_test_dev.ip_cfg.dstip = vos_dottedIP(TRDP_PD_POINT_TO_PONIT_TEST_REMOTE_IP);      /* 目的IP */
    trdp_pd_test_dev.pub_commid = 337;         /* 发布者 commid */
    ...
    tid = rt_thread_create("trdp_pd_point_to_point_test_thread",
                            trdp_pd_point_to_point_test_thread, RT_NULL,
                        (1024 * 6), 20, 5);
    if (tid != RT_NULL)
    {
...
#include <finsh.h>
MSH_CMD_EXPORT_ALIAS(trdp_pd_point_to_point_test, trdp_pd_point_to_point_test, trdp pd point to point test);
#endif
