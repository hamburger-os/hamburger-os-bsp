 ...
 * 返回：逆转后的值
 ******************************************************/
static uint32 bitrev(uint32 in_u32, uint32 bw_u32)
{
    uint32 i;
    uint32 var_u32;
 ...
 * 返回：无
 ******************************************************/
static void create_crc32_table(uint32 poly_u32, uint32 *ptable_u32)
{
  uint32 i;
  uint32 j;
  uint32 c_u32;
...
{
  static const uint8 st_a_crctab8[256] =
  {
...
{    
  static const uint16 st_a_crctab16[256] =
  {
  ...
  while(len > 0U) /* 剩余 */
  {
    crc = (uint16)((uint16)(crc >> 8U) ^ st_a_crctab16[*p_dat_u8 ^ (uint8)crc]);
    len--;        /* 长度减 */
    p_dat_u8++;   /* 指针变化 */
  }        
...
{
  /** 多项式 0x04C11DB7 */
  static const uint32  st_a_CRCTable32[256] =
  {
  ...
  i = (uint32)len & 3U;
  len &= ~3U;
  for(n = 0U; n < (uint32)len; n+= 4U)
  {
    memcpy((void *)&nTmp, (const void *)&p_dat_u8[n], (uint32)4);         /* 拷贝4个字节 */
    crc ^= nTmp;                

    nTemp = st_a_CRCTable32[(uint8)(( crc >> 24U ) & 0xFFU)];    /* 取一个字节，查表 */
  ...
  for(n = 0; n < len; n+= 4)
  {
    ...
    nTemp = st_a_CRCTable32[(uint8)(( crc >> 24 ) & 0xFF)];    /* 取一个字节，查表 */
    crc <<= 8;                                             /* 丢掉计算过的头一个BYTE */
    crc ^= nTemp;                                          /* 与前一个BYTE的计算结果异或 */
...
{
  /** 多项式 0xEDB88320*/
  static const uint32 st_a_crc_32_tab[256] =
  {
  ...
  while (len > 0U)  /* 剩余 */
  {
    t = (oldcrc32 ^ (uint32)(*p_dat_u8)) & (uint32)0xFFU;        /* 计算CRC*/
...
#if 0
uint32 convert_crc32_to_big_endian(uint32 crc32)
{
  uint8 b0, b1, b2, b3;
