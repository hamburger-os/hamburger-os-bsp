                                        ...
                                        uint32_t uwBufferSize)
{
  ...
  else if (hnor->State == HAL_NOR_STATE_READY)
  {
    ...
    p_endaddress      = (uint16_t *)(uwAddress + (2U * (uwBufferSize - 1U)));
    ...
    if (hnor->CommandSet == NOR_AMD_FUJITSU_COMMAND_SET)
    {
      ...
      NOR_WRITE((uwAddress), NOR_CMD_DATA_BUFFER_AND_PROG);
      NOR_WRITE((uwAddress), (uint16_t)(uwBufferSize - 1U));
    }
    else if (hnor->CommandSet == NOR_INTEL_SHARP_EXT_COMMAND_SET)
    {
      /* Write Buffer Load Command */
      NOR_WRITE((uwAddress), NOR_CMD_BUFFERED_PROGRAM);
      NOR_WRITE((uwAddress), (uint16_t)(uwBufferSize - 1U));
    }
    else
    {
    ...
    if (status != HAL_ERROR)
    {
      ...
      if (hnor->CommandSet == NOR_AMD_FUJITSU_COMMAND_SET)
      {
        NOR_WRITE((uwAddress), NOR_CMD_DATA_BUFFER_AND_PROG_CONFIRM);
      }
      else /* => hnor->CommandSet == NOR_INTEL_SHARP_EXT_COMMAND_SET */
      {
        NOR_WRITE((uwAddress), NOR_CMD_CONFIRM);
      }
    }
...
/**
  ...
  */
HAL_NOR_StatusTypeDef HAL_NOR_GetStatus(NOR_HandleTypeDef *hnor, uint32_t Address, uint32_t Timeout, uint32_t osDelay)
{ 
