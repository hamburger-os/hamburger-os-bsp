...
void HAL_HCD_Connect_Callback(HCD_HandleTypeDef *hhcd)
{
    ...
    if (!connect_status)
    {
        ...
        LOG_D("usb connected");
        rt_usbh_root_hub_connect_handler(hcd, OTG_PORT, RT_FALSE);
    }
}
...
void HAL_HCD_Disconnect_Callback(HCD_HandleTypeDef *hhcd)
{
    ...
    if (connect_status)
    {
        ...
        rt_usbh_root_hub_disconnect_handler(hcd, OTG_PORT);
    }
}
...
static int drv_pipe_xfer(upipe_t pipe, rt_uint8_t token, void *buffer, int nbytes, int timeouts)
{
    ...
    while (1)
    {
        ...
        rt_thread_mdelay(1);
        if (HAL_HCD_HC_GetState(&stm32_hhcd, pipe->pipe_index) == HC_NAK)
        {
            LOG_D("nak");
        ...
        else if (HAL_HCD_HC_GetState(&stm32_hhcd, pipe->pipe_index) == HC_STALL)
        {
            LOG_D("stall");
        ...
        else if (HAL_HCD_HC_GetState(&stm32_hhcd, pipe->pipe_index) == URB_ERROR)
        {
            LOG_D("error");
        ...
        else if(URB_DONE == HAL_HCD_HC_GetURBState(&stm32_hhcd, pipe->pipe_index))
        {
            LOG_D("ok");
            ...
            size_t size = HAL_HCD_HC_GetXferCount(&stm32_hhcd, pipe->pipe_index);
...
static rt_err_t stm32_hcd_init(rt_device_t device)
{
    ...
    rt_base_t usb_pwr = rt_pin_get(USBH_POWER_PIN);
    rt_pin_mode(usb_pwr, PIN_MODE_OUTPUT);
    rt_pin_write(usb_pwr, PIN_HIGH);
#endif
    return RT_EOK;
}
