...
void mmcsd_mount(const char* dname)
{
    int ret = 0;
    char blk_dir[32] = {0};
    char *blk_fs = SDCARD_FS;

    /* 挂载文件系统 */
    rt_sprintf(blk_dir, "/mnt/%s/%s", BLK_SDCARD, dname);
    if (dfs_mount(dname, blk_dir, blk_fs, 0, 0) == 0)
    {
        LOG_I("blk %s mount to %s succeed!", dname, blk_dir);
    }
    else
    {
        /* 格式化文件系统 */
        ret = dfs_mkfs(blk_fs, dname);
        if (ret != 0)
        {
            LOG_E("blk %s mkfs %s failed %d!", dname, blk_fs, ret);
            ret = -RT_ERROR;
        }
        else
        {
            /* 再次尝试挂载文件系统 */
            ret = dfs_mount(dname, blk_dir, blk_fs, 0, 0);
            if (ret == 0)
            {
                LOG_W("blk %s mkfs %s mount to %s succeed!", dname, blk_fs, blk_dir);
            }
            else
            {
                LOG_E("blk %s mkfs %s mount to %s failed %d!", dname, blk_fs, blk_dir, ret);
                ret = -RT_ERROR;
            }
        }
    }
